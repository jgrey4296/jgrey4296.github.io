...
De Morgan's Laws 2           - not (A ∧ B) = (not A) ∨ (not B).
*** Structural Rules
Strengthening                - # TODO
...
Production of a new clause implied by two clauses containing complementary literals.
So:
A = a \/ b \/ c
B = d \/ e\/ ~c
gives:
C = a \/ b \/ d \/ e
by mututally incompatible literals c and ~c

If a set of formulas resolve to the empty clause, it is unsatisfiable.

**** Unification
bi-directional pattern matching. Both arguments can contain variables
**** Prolog
...
Propert Class - a class that is not itself as set.
...
* Links
https://www.studysmarter.us/explanations/math/calculus/manipulating-functions/
https://reference.wolfram.com/language/tutorial/AlgebraicManipulationOverview.html
