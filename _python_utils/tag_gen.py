...
load_dir = "docs/site/_posts"
save_dir = "docs/site/_generated/tags"
...
def load_all_posts(dir_name):
    ...
    for post in loadedData:
        if post['tags'] is None:
            continue
        tagSet.update([x for x in post['tags'].split(" ")])
...
def write_yaml_tag_file(target_dir,tag):
    logging.info('Creating Tag File: {}'.format(tag))
    with open(join(target_dir,"{}.md".format(tag)),'w') as f:
        f.write('---\n')
        f.write('layout: tag\n')
        f.write('title: {}\n'.format(tag))
        f.write('tag: {}\n'.format(tag))
...
<ul>
  ...
  {% endfor %}
    ...
    with open(join(dir_name, "index.md"), 'w') as f:
        f.write(index_str)
